...
import os
import random
import finetune.src.load_dataset as load_dataset
...
def main():
    ...
    dataset = load_dataset.get_dataset_from_txt() if args.use_txt else load_dataset.get_dataset_from_zip()
        
    def preprocess_function(examples):
        q1 = [f'query: {x}' for x in examples["question1"]]
        q1_tk = tokenizer(q1, padding="max_length", max_length=args.max_length, truncation=True)
        result = {f"question1_{k}": v for k, v in q1_tk.items()}
        
        q2 = [f'query: {x}' for x in examples["question2"]]
        q2_tk = tokenizer(q2, padding="max_length", max_length=args.max_length, truncation=True)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                loss = get_loss(get_cosing_embeddings(q1_embs, q2_embs), batch["labels"])
